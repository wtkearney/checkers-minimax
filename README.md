...
![If the human player tries to make a move for a given piece when another piece can make a capturing move, a pop-up reminder is displayed and the forced capture move is shown.](./screenshots/capturing-move.png)
...
![Forced capture is implemented for both the AI and human players. From a UI standpoint, if a capture is possible, it is shown as the only legal move for a given selected piece. In this case, a capture is the only valid move shown for the selected black king.](./screenshots/capturing-move-king.png)
